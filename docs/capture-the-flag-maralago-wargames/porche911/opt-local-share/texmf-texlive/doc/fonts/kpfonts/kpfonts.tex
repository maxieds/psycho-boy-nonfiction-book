...
{\large \textsf{Release 3.36}\footnote{This version prepaired by Herbert Voß (hvoss@tug.org)}}
\end{center}
...
Many thanks to
\begin{itemize}
	\item Nicolas Boulenguez (Tests);
	\item Michel Bovani (Fonts);
	\item Daniel Flipo (\LaTeX);
	\item Souraya Muhidine (Translation reviewing)
	\item Peter Rosenberg (\texttt{urw})
	\item Christian Tellechea (package \texttt{xstring})
	\item and the contributors of \texttt{comp.text.tex} and \texttt{fr.comp.text.tex};
	\item Pétiard François (reporting bugs)
	\end{itemize}
