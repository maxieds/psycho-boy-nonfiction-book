...
\bprog
  ...
  1 / 7 + 0.
@eprog\noindent
Immediately a number of decimals of this fraction appear (exactly 38), and
the repeating pattern is $142,857$. The reason is that you have included in
the operations numbers like \kbd{0.}, \kbd{1.} or \kbd{7.} which are
\emph{imprecise} real numbers, hence \kbd{gp} cannot give you an exact
result.

Why 38 decimals by the way? Well, it is the default initial precision.
This has been chosen so that the computations are very fast, and gives
already more accuracy than conventional double precision floating
point operations. The precise value is 128 bits of accuracy by default,
which translates to $128 \log_{10}2 \approx 38.53$ decimal digits, rounded to
$38$. Of course, you can extend the precision (almost) as much as you like as
...
O(x^{17})$; this is completely analogous to \kbd{realprecision} when an exact
...
$\kbd{x} = 0$. When we want them around $\kbd{x} = \kbd{a}$, we replace
...
variable. Try typing \kbd{I = 2}, the error message that you
...
the full set of built-in primes, which goes up to $2^{20} = 1,048,576$ by
default.
...
row, telling us that if primes stopped at $200,000$ as we made \kbd{factor}
believe, then \kbd{n} would be prime. (Or is that a contradiction?) More
seriously, \kbd{n} is not divisible by any prime up to $200,000$.
...
@eprog\noindent
This should take less than 2 seconds. In general, one must use less naive
...
latter, the corresponding elementary operations will be a little slower than
...
(\kbd{gcdext}), solving the Chinese remainder theorem (\kbd{chinese}) and so
on.
...
@eprog\noindent
This time we find 50,886 vectors of the requested length, spanning a
...
\bprog
  ...
  d = F3[2];
@eprog
We reduce the form \kbd{F3} until we reach the unit form (exactly 6 times),
...
\subsec{Defining equations and elements in the field}
...
@eprog\noindent
Anyway, that's $3087$, we don't want to work with such a skewed polynomial!
So, we type
\bprog
  A = polredbest(T)
@eprog\noindent
The \kbd{polredbest} function usually gives a simpler polynomial, where
\emph{red} stands for ``reduction'' and \emph{best} for ``best effort'',
meaning that the function strives to find a good defining polynomial
while guaranteeing that the result is found quickly (in polynomial time).
The polynomial $x^4-x^3-21x^2+17x+133$ defines the same field as $T$, with
somewhat smaller coefficients. Type \kbd{poldisc(A)}, this is better, but
maybe not optimal yet (the index is still $7$).

In fact, had you typed \kbd{nfinit(T, 3)}, \kbd{nfinit} would first have
tried to find a good polynomial defining the same field (i.e.~one with small
index) before proceeding. It's not too late, let's redefine our number field:
\bprog
  NF = nfinit(nf, 3)
@eprog\noindent
...
polynomial $T$ by the better one $A$. (Small wonder, \kbd{nfinit} internally
called \kbd{polredbest}.) The second component enables you to switch
conveniently to our new polynomial.

Namely, call $\theta$ a root of our initial polynomial $T$, and $\alpha$
a root of $A$. These are
...
\kbd{V = apply(polredbest, V)} to reduce them.
...
\noindent will return
\kbd{[607589, 0, 607609]}, which tells you that there are $607,589$ totally
real and $607,609$ totally complex cyclic quartic fields with absolute
...
(more precisely \kbd{R = apply(z -> polredbest(nfsubfields(z, 2, 1)[1]), V)})
