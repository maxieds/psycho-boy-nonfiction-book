			...
			if ( href.search(/https*:\/\S*\/(\S+)\.wikipedia\.org.*/) == 0 || href.search(/\/\S*\/(\S+)\.wikipedia\.org.*/) == 0 )
			{
				...
				if ( validLang && href.search(/\/wiki\/([^\s:#]+)(#?[^\s:]*)$/) > 0 )
				{
					...
					if ( title && title.search(leadingLang) == 0 )
					{
						...
						// rdar://80503341 set an empty string for aria-help so that VoiceOver doesn't duplicate essentially the same word when outputting the title
						linkElements[linkKind][i].setAttribute("aria-help", " ");
					}
				}
			}
...
function appendLanguageLinks( innerDocument, languageInfoList )
{
	...
	for ( var i = 0, langCount = languageInfoList.length;  i < langCount; i++ )
	{
		...
		else						// multiple link candidates
		{
			...
			var dropDownMark = innerDocument.createElement("span");
			dropDownMark.setAttribute("class", "apple_language_link-dropdown");
			dropDownMark.appendChild(innerDocument.createTextNode("▼"));
			langTitleElem.appendChild(dropDownMark);
