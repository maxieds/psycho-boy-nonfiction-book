/*  08-08-2026 11:49 IST [desktop-tierOneCountries]*/
...
if('GFG_AD_Desktop_Stickyunit_1x1_1'==slot.getSlotElementId()||'GFG_AD_Desktop_Stickyunit_1x1_2'==slot.getSlotElementId()){if(!IS_BOTTOM_STICKY_VIEW_ONCE){document.addEventListener('scroll',scrollEventListener);}}});});googletag.cmd.push(function(){var _visibilityQueue={};var _visibilityTimer=null;googletag.pubads().addEventListener('slotVisibilityChanged',function(event){var slotId=event.slot.getSlotElementId();_visibilityQueue[slotId]=event;if(!_visibilityTimer){_visibilityTimer=setTimeout(function(){_visibilityTimer=null;var queue=_visibilityQueue;_visibilityQueue={};Object.keys(queue).forEach(function(id){var evt=queue[id];var slot=evt.slot;var index=activeViewAd.indexOf(slot);var el=slot.getSlotElementId();var blockedItemIdx=blacklistedLineItems.findIndex(curr=>curr.lineItemId==evt.slot?.getResponseInformation()?.sourceAgnosticLineItemId);if(blockedItemIdx>=0){let curr=blacklistedLineItems[blockedItemIdx];updateCurrentAdSlot(id,curr);}else{updateCurrentAdSlot(id,TagAdUnitCopy[id]);}
...
return"";}
function executeParallelAuctionAlongsidePrebid(slots){const filteredSlots=slots.filter(slot=>TempTagAdUnit[slot.getSlotElementId()]?.['headerBidding']=='on');const divIds=filteredSlots.map(slot=>slot.getSlotElementId());var requestManager={adserverRequestSent:false,prebid:false};function biddersBack(){if(requestManager.prebid){sendAdserverRequest();}
return;}
function sendAdserverRequest(){if(requestManager.adserverRequestSent===true){return;}
requestManager.adserverRequestSent=true;googletag.cmd.push(function(){googletag.pubads().refresh(slots);});}
function requestHeaderBids(){pbjs.que.push(function(){pbjs.requestBids({timeout:PREBID_TIMEOUT,adUnitCodes:divIds,bidsBackHandler:function(){pbjs.setTargetingForGPTAsync(divIds);requestManager.prebid=true;setTimeout(function(){biddersBack();},0);}});});}
...
var _gfg_isScrolling=false;var _gfg_scrollTimer=null;var _gfg_idleScheduled=false;window.addEventListener('scroll',function(){_gfg_isScrolling=true;if(_gfg_scrollTimer)clearTimeout(_gfg_scrollTimer);_gfg_scrollTimer=setTimeout(function(){_gfg_isScrolling=false;},200);},{passive:true});const interval=setInterval(function(){if(window.isTabActive==true){if(_gfg_isScrolling)return;if(_gfg_idleScheduled)return;_gfg_idleScheduled=true;if(typeof requestIdleCallback==='function'){requestIdleCallback(function(deadline){_gfg_idleScheduled=false;if(_gfg_isScrolling)return;refreshActiveViewAd();},{timeout:2500});}else{_gfg_idleScheduled=false;refreshActiveViewAd();}
ADS_REFRESH_INTERVAL=0;}},ADS_REFRESH_INTERVAL*1000);function updateCurrentAdSlot(slotId,currSlot){if(TagAdUnit[slotId]){TagAdUnit[slotId].refresh.status=currSlot.refresh.status;TagAdUnit[slotId].refresh.time=currSlot.refresh.time;}}
...
sizes.forEach(size=>{maxHeight=Math.max(maxHeight,size[1]);});});const getDiv=document.getElementById(divId);if(getDiv&&!notApplyOn.includes(divId)){getDiv.style.maxWidth=maxWidth+'px';getDiv.style.maxHeight=maxHeight+'px';getDiv.style.overflow='hidden';getDiv.style.contain='layout style paint';}});}catch(err){console.log("handleAdOverflow => ",err);}}
